...
import { updateBlogPostTitleCommand } from './updateBlogPostTitleCommand.ts'

void describe('updateBlogPostTitleCommand()', () => {
	void it('should published a BlogPost and persist it', async () => {
		...
		const updateTitle = updateBlogPostTitleCommand(findMock, persistMock)

		const adminActorId = testActor()
		const BlogPost = await updateTitle(
			existingBlogPost.$meta.id,
			'New Title',
			v1,
			adminActorId,
		)
		...
		const expectedEvent: Partial<BlogPostTitleChangedEvent> = {
	...
	void it('should throw an error if the BlogPost does not exist', async () => {
		...
		await assert.rejects(
			async () => {
				await updateTitle(id, 'Updated title', v1, testActor())
			},
			{
				name: 'NotFoundError',
